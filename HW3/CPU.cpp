...
int CPU::allControls(string line)
{
	...
	// programi bitirir.
	if(!(isWhichInstruction(line,reg1,reg2,cons)))	
		return 0; 
...
int CPU::findFirst(string line)const
{
	int i;
...
int CPU::isValid(string line,int first)const
{
...
int CPU::spaceForReg(string line,int first)const
{
	first=first+3;
...
int CPU::findFirstInst(string line,int first)const
{
	int i;
	first=first+3;
...
int CPU::controlValid(string line,int regOrConst)const
{
...
int CPU::controlSpace(string line,int regOrConst)const
{
	int i,size,sizetemp;
	string temp;
...
int CPU::register1(string line)const
{
...
int CPU::register2(string line)const
{
...
int CPU::constant(string line)const
{
	string constant;
	int index;
...
int CPU::isWhichInstruction(string line, int reg1, int reg2, int cons)
{
	if(line[0]=='M')
	{
		isMov(line,reg1,reg2,cons);
		setPC(PC+1);
	}	
	else if(line[0]=='A')
	{
		isAdd(line,reg1,reg2,cons);
		setPC(PC+1);
	}
	else if(line[0]=='S')
	{
		isSub(line,reg1,reg2,cons);
		setPC(PC+1);
	}
	else if(line[0]=='J' && line[1]=='M')
	{	
		if(!(isJmp(line,reg1,reg2,cons)))
			return 0;			
	}
	else if(line[0]=='P')
	{
		isPrn(line,reg1,reg2,cons);
		setPC(PC+1);	
	}
	else if(line[0]=='H')
	{	
...
int CPU::isMov(string line, int reg1, int reg2, int cons)
{
	if(cons==-1)
...
int CPU::isAdd(string line, int reg1, int reg2, int cons)
{
	if(cons==-1)
...
int CPU::isSub(string line, int reg1, int reg2, int cons)
{
	if(cons==-1)
		reg[reg1-1]-=reg[reg2-1];
...
int CPU::isJmp(string line, int reg1, int reg2, int cons)
{
	print(line);
	if(reg1==-1)
		setPC(cons);
...
int CPU::isPrn(string line, int reg1, int reg2, int cons)
{
	if(cons==-1)	
...
int CPU::halted()
{
	...
	{
		cout <<"HALT!\n";
		cout <<"R1="<<reg[0]<<",R2="<<reg[1]<<",R3="<<reg[2]<<",R4="<<reg[3]
			<<",R5="<<reg[4]<<endl<<endl;
